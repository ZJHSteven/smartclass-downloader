...
脚本面板有“选择日期”和“下载该日期（队列）”功能。为避免“看起来选了 A 日期，但实际下了 B 日期”，脚本做了两点保证：

- **下拉不再被定时刷新覆盖**：面板会定时刷新列表，但不会再把你手动选择的日期强行改回“最新日期”
- **日期解析更鲁棒**：除 `YYYY-MM-DD` 外，也兼容 `YYYY/MM/DD`、`YYYY.MM.DD`、`YYYY年MM月DD日`，避免某些条目解析失败导致“最新日期判断错误/漏课”

队列下载方面，为了减少“下载不全、过几分钟又继续”的体感问题：

- **队列会等待下载真正结束**（成功/失败/超时）再释放并发计数，避免并发失控触发站点/浏览器节流
- **API 失败降级到后台页** 时，`inflight` 计数由“后台页完成/超时”负责释放，避免重复扣减导致并发计数紊乱
