...
## v0.8.2 - 投递配置区默认收起（2026-03-28）

### 🧩 设置面板
- 调整 `Gopeed / ClassFlow` 模式设置区：默认只显示模式切换，详细配置改为点击“展开配置”后再显示
- 新增设置区摘要文案，收起时也能直接看出当前使用的是 `Gopeed` 还是 `ClassFlow`
- 设置区展开状态会单独记住，避免每次刷新都自动把低频配置重新撑开

### ✅ 测试
- 新增设置区状态与文案单测，覆盖展开状态规范化、摘要文案、按钮文案与模式说明文案
- 保持脚本级 Node 导出能力不变，便于继续直接对 userscript 做纯函数测试
...
### 🚀 下载链路
- 修复 Gopeed 新版参数字段变化导致的文件名/路径传参失效问题（`opt` → `opts`）
- 同步脚本版本号，确保自动更新识别到本次修复

## v0.7.4 - 当前页置顶独立展示（2026-01-11）

### 📌 列表显示
- 当前页课程不再包“日期大框”，直接置顶显示
- 相关推荐仍按日期分组展示

## v0.7.3 - 当前页课程入列（2026-01-11）

### 📌 列表来源
- 当前页面课程会强制加入列表（若推荐中已存在则不重复）
- 当前页条目带“当前页”标签，并在排序中置顶

## v0.7.2 - 日志置顶与配色增强（2026-01-11）

### 🧾 日志展示
- 迷你日志移动到列表上方，并固定高度显示最近 2 条
- 日志配色改为亮色（更易看清）
- 自动展开最新日期改为默认关闭（避免自动弹开）

## v0.7.1 - 日志迷你化展示（2026-01-11）

### 🧾 日志展示
- 日志区域下移到列表下方，默认仅显示最近 3 条
- 完整日志改为折叠面板（需要时再展开查看）
